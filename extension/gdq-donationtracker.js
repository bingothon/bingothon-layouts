...
if (config && config.donationtracker
    ...
    const donationTotalReplicant = nodecg.Replicant('donationTotal');
    const openBidsReplicant = nodecg.Replicant('trackerOpenBids');
    const donationsReplicant = nodecg.Replicant('trackerDonations');
    const prizesReplicant = nodecg.Replicant('trackerPrizes');
    ...
    if (!config.donationtracker.test) {
        ...
        function periodicallySendFakeDonation() {
            sendFakeDonation();
            setTimeout(periodicallySendFakeDonation, 2000 + Math.floor(Math.random() * 20000));
        }
        function updateReplicantsWithFakeData() {
