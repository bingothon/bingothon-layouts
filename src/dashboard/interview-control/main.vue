<template>
    <div>
        Interview:
        <select v-model="selectedInterviewName">
            <option v-for="(interview, i) in allInterviewNames" :key="i" :value="interview">
                {{ interview }}
            </option>
        </select>
        <button @click="updateCurrentInterview">Update Interview</button>
    </div>
</template>
