            ...
            <div class="GameChip">
                <div v-for="(joystick, joystickIndex) in playerJoysticks" :key="joystickIndex" class="FlexContainer">
                    <div>
                        <inline-svg
                            :style="{
                                'margin-left': isCoopGame && joystickIndex > 0 ? `${-9}px` : '0px',
                            }"
                            class="joystick"
                            v-if="joystick"
                            :svg-content="joystick"
                        ></inline-svg>
                    </div>
                </div>
            </div>
...
<script lang="ts">
    ...
    export default class TestGameContainer extends Vue {
        ...
        get isTeamGame(): boolean {
            return store.state.runDataActiveRun.teams.length > 1;
        }
        ...
        get playerCount(): number {
            return store.state.runDataActiveRun.teams[0].players.length;
        }
        ...
        get playerJoysticks(): string[] {
            ...
            let colors = this.playerColors;

            for (let i = 0; i < allPlayers.length; i++) {
                //let player = allPlayers[i];
                let color = colors[i]; // Fetch color for the player based on their index
                let joystickSVG = this.joyStick(color);
