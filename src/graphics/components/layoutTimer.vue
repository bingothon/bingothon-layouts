<template>
    <div class="CountdownContainer FlexContainer">
        <div class="CountdownTimer FlexContainer" :class="{ notRunning }" :style="{ fontSize }" v-html="currentTime"></div>
    </div>
</template>
...
<script lang="ts">
    ...
    @Component({})
    export default class LayoutTimer extends Vue {

        @Prop({ default: "100px" })
        fontSize: string;

        get currentTime(): string {
            return this.splitStringToSpans(store.state.layoutTimer.time);
        }

        get notRunning(): boolean {
            return store.state.layoutTimer.state !== "running";
        }
