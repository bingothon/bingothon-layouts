...
const updateLoopTimer: NodeJS.Timer | null = null;
...
async function fetchBoard(
    token: string,
    host?: string
): Promise<Ori2BingoBoard> {
    return request.get(
        `https://${host || "wotw.orirando.com"}/api/bingothon/${token}`,
        { json: true }
    );
}
...
export class Ori2ExternalBingoboard implements ExternalBingoboardManager {
    ...
    async updateBingoBoard(): Promise<void> {
        ...
        for (const square of board.squares) {
            ...
            cells[index].colors =
        square.completedBy
            .map((universeId) => colorByUniverseId[universeId])
            .join(" ") || "blank";
        }
