...
class TwitchBotWrapper {
    ...
    messageHandler(channel: string, user: tmi.ChatUserstate, message: string, self: boolean): void {
        ...
        if (userCommandName === 'runner' || userCommandName === 'runners' || userCommandName === 'r') {
            ...
            if (runersStr) {
                this.client.say(channel, `Follow the runners! ${runersStr}`).catch((e): void => log.error('', e))
            }
            return
        }
            ...
            if (runDataActiveRunRep.value && runDataActiveRunRep.value.customData) {
                ...
                if (bingotype) {
                    ...
                    if (explanation) {
                        ...
                        this.client.say(channel, explanation).catch((e): void => log.error('', e))
                    }
                }
            }
        }
        ...
        // }
        const knownCommand = twitchChatBotDataRep.value.commands.find((command) => command.name === userCommandName && ['global'].includes(command.commandList))
        if (knownCommand !== undefined && knownCommand.enabled) {
            // check cooldown to not spam chat
            if (now - knownCommand.lastUsed < knownCommand.cooldown * 1000) {
                return
            }
            this.client.say(channel, knownCommand.text).catch((e): void => log.error('', e))
        }
    }

    async disconnect(): Promise<void> {
        await this.client.disconnect().catch((e) => log.error('error on disconnecting, ignoring', e))
        twitchChatBotDataRep.value.state = 'disconnected'
    }
