...
import { GameMode, TwitchChatBotData } from "../../schemas";
  ...
  messageHandler(channel: string, user: tmi.ChatUserstate, message: string, self: boolean): void {
    ...
    if (userCommandName === 'standings' || userCommandName === 'stats' || userCommandName === 'bracket') {
        ...
        if (gameModeRep.value.game === 'sms') {
            this.client.say(channel, "Current Bingo League standings can be found here: https://sms.bingo/standings")
                .catch((e): void => log.error('', e));
        } else {
            this.client.say(channel, "The bracket for the BotW Bingo Bash can be found here: https://challonge.com/botwbingo1")
                .catch((e): void => log.error('', e));
        }
    }
    ...
    if (userCommandName === 'schedule' || userCommandName === 'upcoming') {
        ...
        cooldowns.standings.lastUsed = now;
        if (gameModeRep.value.game === 'sms') {
            this.client.say(channel, "Find the upcoming matches for the league here: https://sms.bingo/schedule")
                .catch((e): void => log.error('', e));
        } else {
            this.client.say(channel, "Find the upcoming matches for the BotW Bingo Bash here: http://bombch.us/DPwY")
                .catch((e): void => log.error('', e));
        }
    }
    const knownCommand = twitchChatBotDataRep.value.commands.find(command =>
      command.name === userCommandName
      && ['global', gameModeRep.value.game].includes(command.commandList));
